...
function abort($header, $response=null){
    header($header);
    if(!is_null($response)){
...
function get_dossiers(Array $repertoire){
    
	$dossiers = [];
	$exclude = [".env", ".htaccess", ".", ".."];
	foreach ($repertoire as $key => $value) {
		if(!strpos($value, ".") && !in_array($value, $exclude)){
			echo $value."\n";
			array_push($dossiers, $value);
		}
	}
